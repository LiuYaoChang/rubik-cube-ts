...
export class THREExDomEvent {
    ...
    */
    ...
    unbind(object3d: Object3D, eventName: EventNames, callback: Function) {
      ...
      const index = this._boundObjs.findIndex((obj3d) => (obj3d === object3d));
    ...
    _onMove(mouseX: number, mouseY: number, event: MouseEvent) {
      ...
      const intersects = intersectObjects(this._boundObjs, this._camera, vector);
    ...
    _onEvent(eventName: EventNames, mouseX: number, mouseY: number, event: MouseEvent) {
      ...
      const face: Face = intersect.face as Face;

      const objectCtx = this._objectCtxGet(object as Object3D);

      if (!objectCtx) return;
      this._notify(eventName, object, event, face);
    }

    _notify(eventName: EventNames, object3d: Object3D, event?: MouseEvent, targetFace?: Face) {
  ...
  function intersectObjects(objects: Object3D[], camera: Camera, vector: Vector3) {
